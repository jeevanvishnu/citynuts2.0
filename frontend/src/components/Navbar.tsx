                            ...
                            <div className="flex-1 flex flex-col px-6 border-l border-primary/10">
                              ...
                              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                {megaMenuData[activeCategory]?.subs.map((sub: string) => (
                                  <Link
                                    key={sub}
                                    to={`/products?category=${activeCategory}&type=${sub.replace(' ', '')}`}
                                    onClick={() => setIsProductsHovered(false)}
                              ...
                              <div className="mt-auto pt-6">
                                <Link to={`/products?category=${activeCategory}`} onClick={() => setIsProductsHovered(false)} className="text-[11px] font-bold text-primary hover:text-primary-dark flex items-center gap-1 transition-colors uppercase">
            ...
            {user ? (
              ...
              <>
                <button
                  ...
                  className={`hidden sm:flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary/30 text-primary hover:bg-primary hover:text-white transition-all duration-300 font-medium text-sm cursor-pointer ${location.pathname === '/login' ? 'bg-primary text-white border-primary' : ''}`}
                >
                  <User size={18} weight="bold" />
                  <span>Login</span>
                </button>
                <button
                  onClick={() => navigate('/login')}
                  className={`sm:hidden p-2 rounded-full hover:bg-primary/10 text-dark/80 hover:text-primary transition-all duration-200 active:scale-95 cursor-pointer ${location.pathname === '/login' ? 'bg-blush text-primary' : ''}`}
                  aria-label="Login"
                >
                  <User size={22} weight="bold" />
                </button>
